...
# ─────────────────────────────────────────────────────────────────────────────
# Workload chart endpoint — returns 48-point time-series (24h actual + 24h predicted)
# ─────────────────────────────────────────────────────────────────────────────

@router.get("/workload/chart")
async def get_workload_chart(current_user: CurrentUser, tenant_id: TenantID, db: DBSession):
    """
    Returns the 48-hour workload chart data (24h actual activity + 24h predicted).
    Data is generated by sync_ml_insights and stored as a workload_chart MLPrediction.
    Falls back to an empty chart with a flag if no data has been generated yet.
    """
    from sqlalchemy import select
    from app.models.ml_prediction import MLPrediction
    ...
    pred = (await db.execute(
        ...
        .where(
            ...
            MLPrediction.prediction_type == "workload_chart",
        )
        .order_by(MLPrediction.predicted_at.desc())
        .limit(1)
    )).scalar_one_or_none()
    ...
    if pred is None:
        return APIResponse(data={"points": [], "is_fallback": True})

    points = (pred.output_data or {}).get("points", [])
    return APIResponse(data={
        "points":      points,
        "is_fallback": False,
        "generated_at": pred.predicted_at.isoformat() if pred.predicted_at else None,
        "confidence":   pred.confidence,
    })


# ─────────────────────────────────────────────────────────────────────────────
# On-demand ML sync — lets the UI trigger a fresh prediction run
# ─────────────────────────────────────────────────────────────────────────────

@router.post("/sync")
async def trigger_ml_sync(current_user: CurrentUser, tenant_id: TenantID, db: DBSession):
    """
    Trigger a full ML insight sync for this tenant in the background.
    ...
    async def _run():
        try:
            ...
            logger.info(f"[ml/sync] Background sync complete for tenant {tenant_id[:8]}: {result}")
        except Exception as exc:
            logger.error(f"[ml/sync] Background sync failed for tenant {tenant_id[:8]}: {exc!r}")

    asyncio.create_task(_run(), name=f"ml-sync-{tenant_id[:8]}")

    return APIResponse(
        data    = {"status": "running", "tenant_id": tenant_id},
        message = "ML sync started — predictions will update in a few seconds",
    )
